...
public class TestOrganizationWithEmailId extends BaseClass {
	...
	public void testNewOrganization() throws IOException, InterruptedException
	{
		...
		//navigate new  organization
				...
				cnop.createNewOrganization(memberName, emailId);
				
		//wait for element		
		wlib.waitForElementandclick(hp.getOrganizationLink());
		
		//creating another organization with EmailID
		
			op=hp.navigateToOrganizationsPage();
				cnop=op.navigateToCreatingNewOrganizationPage();
				 OrganizationInformationPage oip = cnop.createNewOrganization(driver, orgName, emailId);
